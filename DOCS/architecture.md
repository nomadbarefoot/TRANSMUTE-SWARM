...
- **`pyproject.toml`** registers a `swarm` console script pointing to `cli.__main__:app`.
- **`cli/`** contains:
  - `cli/console.py`: shared Rich console + theming helpers.
  - `cli/commands/clean.py`: `swarm scan` and `swarm clean` commands, which:
    - import `agents.calcinator`
    - call `scan_artifacts`, `do_archive`, `do_purge`, and `do_flag`
    - render results in Rich tables and panels.
- **`agents/calcinator.py`** is the artifact lifecycle manager:
  - scans `cogs/`, `solutions/`, `results/`, and `discoveries/` against `config/task_registry.yaml`
  - categorizes artifacts as orphaned/archivable/flag-only
  - archives results/logs to `results/archive/` and discoveries to `discoveries/archive/`
  - exposes a small CLI (`scan`, `archive`, `purge`, `flag`, `clean`) and an `auto_scan` hook used by `cog_manager`.
- **`swarm` / `swarm.sh`** in the repo root:
  - activate `.venv` if present
  - delegate to the installed `swarm` entrypoint (`exec swarm "$@"`).
